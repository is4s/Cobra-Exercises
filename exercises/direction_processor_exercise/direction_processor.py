...
class DirectionMeasurementProcessor(StandardMeasurementProcessor):
    ...
    def __init__(
       ...
       raise NotImplementedError("DirectionProcessor needs initialized. Additional parameters from the plugin/config may be required.")

    def receive_aux_data(self, aux: list[Message | None]) -> None:
        raise NotImplementedError("The DirectionProcessor may need to accept some aux data.")
    ...
    def generate_model(
        ...
        def h(x: NDArray[float64]) -> NDArray[float64]:
            raise NotImplementedError("Must implement the non-linear measurement function.")
            #return zeros((m, 1))
